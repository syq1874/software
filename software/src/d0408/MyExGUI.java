...
public class MyExGUI {
    ...
    private JFrame mainWindow = new JFrame("四则运算");
    ...
    private JTextField JTFUserName = new JTextField(15);//10的单位不是px 而是指定列数
    private String[] operationType = {"初级","高级"};
    ...
    public MyExGUI(){
        ...
        JCBOperationSelect.setPreferredSize(new Dimension(60,25));       //当selectPanel.setLayout那句存在时，这里的设置无效
        ...
        mainWindow.setSize(1000,500);//设置窗体大小
        ...
        for(int i = 0;i < 10;i++)
        {
            switch(chara)
            {
             case "初级":
                 ...
                 break;
             case "高级":
                 questions[i] = new Multiplication2(n);
                 JLBQuestions[i].setText(questions[i].printQuestion());
                 break;
             default:
                    JFrame jf = new JFrame();
